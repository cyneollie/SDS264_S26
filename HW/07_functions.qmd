...
Since we are filtering before doing the calculation, we remove the NA row first, which then also alter the rescaling range. The scaling is then based on a different dataset than the original column. 
...
Since the two vectors do not have equal length (1:6 and 1:3), the shorter one gets recycled.
...
`x` and `w` must be the same length WHEN FALSE
wt_mean(1:6, 1:3) : `x` and `w` must be the same length WHEN TRUE
...
is.na(x) returns us back a vector, not a single value which what the if function wants
...
sort = sort, because we already defined it in our argument that the default is TRUE. count() already has a built in sort argument.

We are not embracing df because embrace is only for column name. Our dataframe which is in the form of tibble or dataframe is already a regular object that R recognise. 

we do not need n because count will automatically make it for us. 
...
pick() provides a way to easily select a subset of columns from our data while inside a "data masking" function such as mutate() or summarise()
...
```{r}
screenshare <- function(names) {
  sample(names, 1)
}

screenshare(c("Mary", "Isimbi", "Lelo", "Cynthia", "Kaisa"))
```
...
mpg |>
  ...
  summarise(meanhwy = mean(hwy),
            meancty = mean(cty),
            se_fun_hwy = se_fun1(hwy),
            se_fun_cty = se_fun1(cty))
```
...
se_na(c(1:5, NA), includeNAs = TRUE)
flights |>
  group_by(carrier) |>
  summarize(mean_arr_delay = mean(arr_delay, na.rm = TRUE),
            mean_dep_delay = mean(dep_delay, na.rm = TRUE),
            se_arr = se_na(arr_delay, includeNAs = TRUE),
            se_dep = se_na(dep_delay, includeNAs = TRUE))
```
...
```{r}
se_na_5 <- function(x, na.rm = FALSE) {        # MAKING na.rm = FALSE the default value.
  if (na.rm) {
    x <- na.omit(x)
  }
  sqrt(var(x) / length(x))
...
x1 <- c(1, 1, 3, NA, NA, NA, NA)
y1 <- c(NA, 1, 2, 3, NA, NA, NA)
...
We get a warning message "longer object length is not a multiple of shorter object length".

```{r}
#| error: true

both_na_7 <- function (x, y) {
  if (length(x) != length(y)) {
    stop("'x' and 'y' must be the same length", call. = FALSE)
  } else {sum(is.na(x) & is.na(y))                             # WE WANT & SIGN AND NOT == SIGN
  }
}
...
```{r}
flights |>
  summarise(n_dep_delay = sum(is.na(dep_delay)),
            n_arr_delay = sum(is.na(arr_delay)),
              n_both = both_na_7(dep_delay, arr_delay)
  )
```
...
f1 is to find the duration or length in minutes between two given times.
f1 can be named : duration_mins

f2 is to turn cm to inch.
f2 can be named : to_inches

f3 is to recode several categories into one general group "non answer"
f3 can be named to : groupto_na
...
foo(c(8, 15, 13, 12, -100, -101))


# my answer: 
# so, 15-8 = 7; 13-15 = -2
# this first part of the function takes the difference of a value from x from the value that comes before it... 
# so like (n+1 - n) kind of thing.

# so automatically the second part of the code
# which is sum(diff < 0)
# counts how many times negative difference appear...
...
foo2 <- function(x) {
  if (is.na(sum(x))) {
    stop("there is missing value(s) in the input vector")
  } else {diff <- x[-1] - x[1:(length(x) - 1)]
  sum(diff < 0)
  }
}

foo2(c(8, 15, 13, 12, NA, -100, -101))
```
...
greet(time = "2018-05-03 16:38:01 CDT") 
greet(time = "2018-05-03 04:38:01 CDT") 
greet(time = Sys.time())
```
...
```{r}
summary6_modif <- function(data, var, removeNAs = FALSE) {
  if (removeNAs) {
    data <- 
      data |>
      na.omit({{ var }})
...
mpg |>
  summary6_modif(hwy, removeNAs = TRUE)

gss_cat |>
  summary6_modif(age, removeNAs = TRUE)
```
...
```{r}
summary6_modif2 <- function(data, group_vars, var, removeNAs = FALSE) {
  if (removeNAs) {
    data <- 
      data |>
      na.omit({{ var }})
...
mpg |>
  summary6_modif2(drv, hwy, removeNAs = TRUE)

partyid_table <- gss_cat |>
  summary6_modif2(partyid, age, removeNAs = TRUE)
```
...
last_value(c(1, 1, 3, NA, NA, NA, NA))
```
...
plotting <- function(df, xvar, yvar, labelvar, sizevar) {
  ggplot(data = df, mapping = aes(x = {{ xvar }}, y = {{ yvar}})) +
    geom_point(aes(size = {{sizevar}})) +
    geom_smooth(method = lm) +
    geom_label_repel(aes(label = {{ labelvar }})) +
    labs(title = "Mean vs IQR")
}

plotting(partyid_table, mean, IQR, partyid, n)
