...
```{sql, connection=con}
SELECT Year, TAIL_NUMBER
FROM flightdata 
GROUP BY Year, TAIL_NUMBER
LIMIT 0, 10;
```
...
SUM(CASE WHEN ArrTime IS NULL THEN 1 ELSE 0 END) / SUM(1) AS prop_cancelled
FROM flightdata
WHERE dest = 'MSP' AND 
      origin IN ('JFK', 'LGA', 'EWR') AND 
      Reporting_Airline IN ('DL', '9E', 'AA', 'MQ', 'EV', 'OO', 'UA')
GROUP BY origin, 
    CASE
        WHEN Reporting_Airline IN ('DL', '9E') THEN 'Delta +'
        WHEN Reporting_Airline IN ('AA', 'MQ') THEN 'American +'
...
```{sql, connection=con, output.var = adapting2}
SELECT
origin,
dest,
SUM(1) AS n,
SUM(CASE WHEN ArrTime IS NULL THEN 1 ELSE 0 END) / SUM(1) AS prop_cancelled
FROM flightdata
WHERE Year = 2023 AND origin IN ('JFK', 'LGA', 'EWR')
...
LEFT JOIN planes AS p ON f.TAIL_NUMBER = p.tailnum
JOIN airlines AS a ON f.Reporting_Airline = a.carrier
WHERE f.Year = 2023 AND 
      origin IN ('JFK', 'LGA', 'EWR')
GROUP BY Reporting_Airline
HAVING COUNT(p.year) > 0
