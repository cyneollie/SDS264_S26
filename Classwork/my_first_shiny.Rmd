...
#  geom_histogram(aes(y = after_stat(density))) +
 # geom_density()
inputPanel(
  selectInput("eruptions_breaks", label = "Number of bins:",
              choices = c(10, 20, 35, 50), selected = 20),
  
  sliderInput("den_adjust", label = "Density adjustment:",
              min = 0.2, max = 2, value = 1, step = 0.2)
)
```
...
renderPlot({
  ggplot(faithful, 
         aes(x = eruptions)) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = as.numeric(input$eruptions_breaks)) +
  geom_density(adjust = input$den_adjust, color = "blue")
})
```
