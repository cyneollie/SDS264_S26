...
avg_taxiin + avg_taxiout AS avg_totaltaxi
FROM flightdata
WHERE Year = 2019 AND Month = 9 AND DayofMonth = 30 AND origin = 'MSP'
...
first number = offset. 'start at first row'
second number = the number of rows that we want. we want 10 rows in total
...
rename it "AS" : rename the variable
flight_num <int>  path <chr>
4628	            (BDL to LGA)			
4628	            (BDL to LGA)			
4628	            (BDL to LGA)			
4628	            (BDL to LGA)			
...
select > from > any sort of filtering, grouping etc. 
...
COUNT(*) and SUM(1) = count the rows
...
carrier <chr>     numFlights <S3: integer64>    firstDepartureTime <S3: hms>
9E	              2	                            05:57:00		
AA	              14  	                      	04:55:00		
B6	              10	                        	06:03:00		
DL	              16		                        05:58:00		
...
But where wouldnt work to filter the variable that we created/derived. 

in the example above, let's say we want to filter only "relatively" frequent flights. 
HAVING numFlights > 365 * 2
we want to filter flights that averaging at least two flights per day. 
...
This join will be inner join.

Left join = start with data. left join other_data. left join make sure we keep all rows from the first data
inner join = i have to have data in both data for both tables.  
...
the asterisk (*) is a wildcard character used within a SELECT statement to retrieve all columns from a specified table
